...
public class MainActivity extends Activity implements Button.OnClickListener{
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        bth= new BluetoothHandler();
        bth.startbt();
        IntentFilter filter = new IntentFilter(BluetoothDevice.ACTION_FOUND);
        registerReceiver(bth.getmReceiver(), filter); // Don't forget to unregister during onDestroy
    ...
    public void onClick(View v) {
        if ( (Button)v == btnServer) {
            ...
            bth.getAdapter().startDiscovery();
        } else if ( (Button)v == btnScan) {
            for(String s : bth.getList()){
                output.append(s + "\n");
            }
